  ...
  return (
    <div className="absolute bottom-3 left-3 z-30 fb-glass-chrome rounded-md border border-[color:var(--glass-chrome-border)] shadow-md flex items-stretch overflow-hidden">
      <button
        ...
        className="h-7 w-7 inline-flex items-center justify-center text-[var(--ink-70)] hover:bg-[var(--surface-sunken)]"
        title="Zoom out (⌘[)"
        aria-label="Zoom out"
      >
      ...
      <button
        onClick={reset}
        className="px-2 inline-flex items-center justify-center text-[10px] font-mono tabular-nums text-[var(--ink-90)] hover:bg-[var(--surface-sunken)] min-w-[44px]"
        title="Reset view (⌘0)"
      >
        {Math.round(zoom * 100)}%
      </button>
