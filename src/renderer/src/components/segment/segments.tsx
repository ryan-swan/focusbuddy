import SegmentShell, { type SegmentDef } from './SegmentShell'
...
// The four top-level segments. Each is a focused area with a home of app tiles,
// reusing the existing views inline. The global sidebar is the single persistent
// menu, so a segment renders its CONTENT only (via SegmentShell) in the centre
// panel; selecting an app drives the view store so the sidebar stays in step.
// PlexiOffice keeps its own richer content component (PlexiOfficeShell) because of
// its document create/open affordances.

const DESK_DEF: SegmentDef = {
  wordmark: 'PLEXIDESK',
  title: 'PlexiDesk',
...
export function PlexiDeskShell({ activeApp }: { activeApp?: string } = {}): JSX.Element {
  const goPlexiDesk = useViewStore((s) => s.goPlexiDesk)
  return <SegmentShell def={DESK_DEF} activeApp={activeApp ?? null} onOpenApp={(app) => goPlexiDesk(app)} />
}

export function PlexiPeopleShell({ activeApp }: { activeApp?: string } = {}): JSX.Element {
  const goPlexiPeople = useViewStore((s) => s.goPlexiPeople)
  // People defaults to its home tiles when no app is deep-linked.
  return <SegmentShell def={PEOPLE_DEF} activeApp={activeApp ?? null} onOpenApp={(app) => goPlexiPeople(app)} />
}

export function PlexiBrainShell({ activeApp }: { activeApp?: string } = {}): JSX.Element {
  const goPlexiBrain = useViewStore((s) => s.goPlexiBrain)
  return <SegmentShell def={BRAIN_DEF} activeApp={activeApp ?? null} onOpenApp={(app) => goPlexiBrain(app)} />
}
