...
| Decisions panel (PlexiBrain) | Done | dc6a8c6 | Lists every live Decision with a live at-risk status (rose/emerald pill), owner/state/linked-object count, Open-desk and Retire actions, and an honest empty state. Backed by decisions:withRisk over a shared objectHealth helper. Tester-verified. |
