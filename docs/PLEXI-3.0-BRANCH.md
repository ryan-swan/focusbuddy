...
## The sync-and-depth wave (third batch)

Safety first: the preview build now refuses to run workspace sync or cloud-docs sync when its configured signal host is the production one (lib/previewGuard.ts over a new app:isPreviewBuild IPC). Signing into the preview with a real account can no longer touch that account's synced data. Pointing a build at a local server re-enables sync, which is how the sync ladder is developed and tested: local focusbuddy-signal, throwaway accounts, production untouched.

Sync ladder rung 1 is the calendar. time_blocks carries the same sync bookkeeping as nodes and widgets (sync_rev, needs_sync with a guarded dirty trigger) plus trashed_at tombstones so deletes propagate rather than resurrecting. Recurring occurrences now use deterministic ids (seriesId@startMs) inserted with ON CONFLICT DO NOTHING, which makes materialisation idempotent across devices and lets a tombstoned occurrence block its own regeneration everywhere. The client sync module orders nodes first for foreign keys, survives a bad row without aborting the batch, and skips unknown future item types; the renderer refreshes the calendar when a pull applies. The server-side item-type union is widened in the local focusbuddy-signal checkout only; nothing is deployed.

Local documents got real comments. The doc editor's comment panel previously worked only for live/collaborative docs; ordinary local docs now have a doc_comments table (cascading with the document), a docComments IPC, and full wiring in DocumentEditorView: select text, add an anchored comment (same Tiptap mark and amber highlight the live editor uses), reply in threads, click a thread to jump to its anchor, all working signed out. Resolution controls remain with the live-doc floating panel for now; local rows already carry the resolved flag.

Mail got keyboard triage: j and k open the previous or next thread, r seeds a reply, a archives via a real IMAP move (the archive mailbox is detected from the special-use attribute, with fallbacks, created if missing) with optimistic removal that restores honestly on failure. A quiet hint line above the inbox teaches the keys.

## Still open from the review, in priority order

The sync ladder has its first new rung (calendar); next are documents/tables/files coverage decisions (documents already have the flag-gated cloudDocs path — decide whether to promote it or fold docs into workspaceSync), then cross-member org sync, then web/mobile with push. Mail OAuth and multi-account, TURN for calls, AI action-vocabulary widening (consult ai-proposal-owner and proposal-applier-owner first), chat mentions and channel semantics, calendar recurrence and reminders, the unified notification center, per-seat billing, the shared Modal primitive adoption sweep across the 66 overlay components, the stone-* token conversion of the top ten files, and internationalization. Mail OAuth/multi-account remains blocked on the operator's Google Cloud credentials, and full mail search needs a local index (the recent-inbox cache is a stopgap). Chat search still needs a server endpoint. Keyboard-only mail triage is the remaining power-user ask.
