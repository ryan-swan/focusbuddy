...
| F-8 | **Chat is fully broken on PlexiDesk credits**: the managed-Plexii proxy rejects streaming with 400 "Streaming is not supported on PlexiDesk credits", and the chat + workspace-ask paths always streamed — any user routed onto credits (the new first-run default) cannot chat at all | reproduced live 2026-08-26 (operator screenshot); this branch carries a client-side non-streaming fallback when the credits client is active (`anthropic.ts`, commit e244b39d) | proxy-side: support streaming; until then the fallback stands |
| F-9 | **Keyboard selection in the chat mention picker was dead**: ReactRenderer `.ref` resolves null/stale on the current React/tiptap pairing, so ↑/↓/Enter/Tab in the "@" popup fell through to the composer — Enter even sent the half-typed mention as a chat message; mouse-only selection was the silent requirement | reproduced via CDP driving 2026-08-26; fixed on this branch (`MentionList.bindKeys` self-registration, commit db2888d0) | keep the branch fix; consider upstreaming the same pattern to SlashMenuList if it shares the wiring |
